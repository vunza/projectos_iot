...
if (!!window.EventSource) {
    ...
    source.addEventListener('mac_addr', function (e) {

        var sz_json = JSON.parse(e.data);
        //console.log(sz_json);
        // Checar o texo JSON
        if (Object.entries(sz_json).length === 0) {
            console.log("JSON vazio!!");
            return;
        }

        document.getElementById('lgd').innerHTML = '<b>&nbsp;' + sz_json.Nome + '&nbsp;</b>';
...
function restrictSpecialChars(input) {
    ...
    if(input.value == get_dev_name || input.value.length <= 0){
        ...
        document.getElementById("div_guardar_nome").querySelector('svg').style.stroke = "#FFFFFF";
    }
}

// Envia comando aos clientes para activar modo OTA, para carregar SKETCH/SPIFFS 
